...
class OutboundMessageTowerListenerCommand extends Command
{
    ...
    public function __construct(string $name = null, OutboundMessageTower $outboundMessageTower, $towerPollingInterval)
    {
        ...
        $this->towerPollingInterval = $towerPollingInterval;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!preg_match("/^[a-z0-9-_]+$/i", $channelName)) {
            throw new OutboundMessageTowerException('Invalid channel name. Must match regex /^[a-z0-9-_]+$/i');
        }

        $output->writeln('Listening for Outbound Message Tower broadcasts...');
        $sleepFor = $this->towerPollingInterval > 0 ? (int)$this->towerPollingInterval * 1000 : null;
